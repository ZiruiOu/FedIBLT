...
class Fermat {
 public:
  ...
  Fermat(int r, int c, uint32_t _id[rr][cc], uint32_t _fingerprint[rr][cc],
         uint32_t _counter[rr][cc], double _value[rr][cc],
         uint32_t _hash_fing[dd], uint32_t _buckets[rr][dd]) {
    array_num = r;
    entry_num = c;
    id = _id;  // Key Sum
    fingerprint = _fingerprint;
    counter = _counter;  // Counter
    value = _value;      // Value Sum
    hash_fing = _hash_fing;
    buckets = _buckets;  // Hash Value
    use_fing = 0;
  }

  Fermat() {
    use_fing = 1;
    if (use_fing) hash_fp = new BOBHash32(357);
    hash = new BOBHash32[rr];
    for (int i = 0; i < rr; ++i) hash[i].initialize(357 + i + 1);
  }
  ...
  void insert(uint32_t flow_id, uint32_t cnt, vt v) {
    if (use_fing) {
      ...
      for (int i = 0; i < array_num; ++i) {
        uint32_t pos =
            hash[i].run((char *)&flow_id, sizeof(uint32_t)) % entry_num;
        id[i][pos] = (id[i][pos] + mulMod(flow_id, cnt, PRIME_ID)) % PRIME_ID;
        fingerprint[i][pos] =
            ((uint64_t)fingerprint[i][pos] + mulMod32(fing, cnt, PRIME_FING)) %
            PRIME_FING;
        counter[i][pos] += cnt;
        value[i][pos] += v;
      }
  ...
  void insert_one(uint32_t flow_id, vt v) {
    if (use_fing) {
      uint32_t fing =
          hash_fp->run((char *)&flow_id, sizeof(uint32_t)) % PRIME_FING;
      for (int i = 0; i < array_num; ++i) {
        uint32_t pos =
            hash[i].run((char *)&flow_id, sizeof(uint32_t)) % entry_num;
        id[i][pos] = (id[i][pos] + (flow_id % PRIME_ID)) % PRIME_ID;
        fingerprint[i][pos] =
            ((uint32_t)fingerprint[i][pos] + (uint32_t)fing) % PRIME_FING;
        counter[i][pos]++;
        value[i][pos] += v;
      }
    } else {
  ...
  void delete_in_one_bucket(int row, int col, int pure_row, int pure_col) {
    ...
    if (use_fing)
      fingerprint[row][col] =
          fingerprint[row][col] - fingerprint[pure_row][pure_col];
  ...
  bool verify(int row, int col, uint32_t &flow_id, uint32_t &fing) {
    ...
    if (use_fing && fingerprint[row][col] % cnt) {
      return false;
    }

    flow_id = id[row][col] / cnt;

    if (use_fing) fing = fingerprint[row][col] / cnt;

    if (!(buckets[row][flow_id] == col)) flag1 = false;
    if (use_fing && !(hash_fing[flow_id] == fing)) flag2 = false;
